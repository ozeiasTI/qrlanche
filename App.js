...
import { View, Text, TextInput, Button, StyleSheet, Alert, FlatList, Platform, ScrollView, ActivityIndicator } from 'react-native';
...
function ScannerScreen({ route, navigation }) {
  ...
  const [countServidor, setCountServidor] = useState(0);
    ...
    try {
      ...
      const countsRes = await db.getAllAsync(
        `SELECT 
          ...
          SUM(CASE WHEN a.curso = '666 - Servidor' THEN 1 ELSE 0 END) as servidor,
          SUM(CASE WHEN a.curso NOT LIKE '0403%' AND a.curso != '666 - Servidor' THEN 1 ELSE 0 END) as superior
         FROM registros r
      ...
      setCountServidor(countsRes[0].servidor || 0);
    ...
    <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
      ...
      <View style={styles.overlay}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-around', width: '100%' }}>
          ...
          <View style={{ alignItems: 'center' }}>
            <Text style={styles.counterLabel}>Servidor</Text>
            <Text style={styles.counterValue}>{countServidor}</Text>
          </View>
          <View style={{ alignItems: 'center' }}>
...
function HistoryScreen() {
  ...
  const [stats, setStats] = useState({ tecnico: 0, superior: 0, servidor: 0 });
  ...
  const loadData = async (sessao = 'todas', nomeFiltro = '') => {
    let whereClauses = [];
    let params = [];
    const lowerName = nomeFiltro.trim().toLowerCase();
    if (sessao !== 'todas') {
      whereClauses.push('r.sessao = ?');
      params.push(sessao);
    }
    if (lowerName) {
      whereClauses.push('lower(a.nome) LIKE ?');
  ...
  useEffect(() => {
    loadData(selectedSession, nameFilter);
  }, [selectedSession, nameFilter]);
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      page.drawText(`Servidor: ${stats.servidor}`, { x: 170, y, size: 12, font });
      page.drawText(`Superior / Outros: ${stats.superior}`, { x: 300, y, size: 12, font });
      page.drawText(`Total: ${records.length}`, { x: 450, y, size: 12, font });
      y -= 25;

      // Cabeçalho
      page.drawText('Sessão', { x: 50, y, size: 10, font, color: rgb(0, 0, 1) });
      page.drawText('Nome', { x: 150, y, size: 10, font, color: rgb(0, 0, 1) });
      page.drawText('Curso', { x: 300, y, size: 10, font, color: rgb(0, 0, 1) });
      page.drawText('Data/Hora', { x: 470, y, size: 10, font, color: rgb(0, 0, 1) });
      y -= 15;
      ...
      records.forEach((item) => {
        ...
        page.drawText(String(item.nome || item.codigo).substring(0, 25), { x: 150, y, size: 9, font });
        const cursoResumo = item.curso
          ? item.curso === '666 - Servidor' || item.curso.toLowerCase().includes('servidor')
            ? 'Servidor'
            : item.curso.includes('0403')
              ? 'Técnico'
              : 'Superior'
          : 'N/A';
        page.drawText(cursoResumo, { x: 300, y, size: 9, font });
        page.drawText(item.datahora, { x: 470, y, size: 9, font });
        y -= 12;
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        <Text style={styles.statsText}>Técnico: {stats.tecnico} | Superior: {stats.superior} | Servidor: {stats.servidor}</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.listItem}>
            <View style={{ flex: 1 }}>
              <Text style={styles.listText}>{item.nome || item.codigo}</Text>
              <Text style={styles.listCode}>Sessão: {item.sessao}</Text>
              <Text style={styles.listDate}>Matrícula: {item.codigo}</Text>
              <Text style={styles.listDate}>Curso: {item.curso?.substring(0, 40)}...</Text>
              <Text style={styles.listDate}>Data: {item.datahora}</Text>
            </View>
          </View>
        )}
      />
...
function ManageStudentsScreen() {
  ...
  const importStudents = async () => {
    ...
    try {
      ...
      const insertPromises = json.map(async (row) => {
        ...
        if (curso === '666' || curso.toLowerCase().includes('servidor')) {
          curso = '666 - Servidor';
        }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ marginVertical: 10 }}>
        <Button title="Importar Alunos (Planilha)" onPress={importStudents} disabled={isImporting} />
      </View>
      {isImporting && (
        <View style={{ marginBottom: 10, alignItems: 'center' }}>
          <ActivityIndicator size="large" color="#0078D7" />
          <Text style={{ marginTop: 8, color: '#0078D7' }}>Importando planilha, aguarde...</Text>
        </View>
      )}
      <View style={{ marginVertical: 10 }}>
...
function HelpScreen() {
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.helpText}>3. O app diferencia cursos Técnicos (0403), Superiores e Servidores (666).</Text>
